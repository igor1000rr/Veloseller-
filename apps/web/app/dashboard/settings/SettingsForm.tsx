...
export default function SettingsForm({ initial, telegramDeeplink }: { initial: Initial; telegramDeeplink: string | null }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white border border-slate-200 rounded-2xl p-6">
      <section>
        <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500 mb-3">Профиль</h2>
        ...
        <label className="block">
          <span className="block text-sm font-medium text-slate-700 mb-1">Часовой пояс</span>
          <input
            value={timezone}
      ...
      <section>
        <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500 mb-3">Уведомления</h2>
        ...
        <label className="block ml-7">
          <span className="block text-xs text-slate-600 mb-1">Telegram Chat ID</span>
          <input
            value={chatId}
            onChange={(e) => setChatId(e.target.value)}
            placeholder="Привяжется автоматически"
          ...
          {telegramDeeplink ? (
            <a href={telegramDeeplink} target="_blank" rel="noopener noreferrer"
              ...
              Подключить Telegram автоматически
            </a>
          ) : (
            <span className="text-xs text-slate-500 mt-1 block">
              Или введите Chat ID вручную. Получить можно через <a href="https://t.me/userinfobot" target="_blank" className="text-violet-600 hover:underline">@userinfobot</a>.
            </span>
          )}
        </label>
      </section>
