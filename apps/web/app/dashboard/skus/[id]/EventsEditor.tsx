...
function EventsTable({
  ...
  function saveAll() {
    for (const e of rows) {
      ...
      if (changed) {
        onUpdate(e.id, {
          title: d.title.trim().slice(0, 100),
      ...
      <div className="w-full max-w-md rounded-2xl border border-line bg-paper p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          <button
            type="button"
            disabled={!valid}
            onClick={() => onSubmit({ title: title.trim().slice(0, 100), startDate, endDate: endDate || null, comment: comment.trim() ? comment.slice(0, 1000) : null })}
