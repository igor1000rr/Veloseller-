...
const segments = [
  ...
  { name: "steady", value: 42, color: "#0ea5e9" },
...
export default function DashboardPreview() {
  return (
    <div className="relative">
      ...
      />
      <div className="rounded-2xl border border-line bg-paper overflow-hidden shadow-[0_30px_80px_-25px_rgba(10,13,10,0.25)]">
        {/* Top bar */}
        <div className="flex items-center justify-between border-b border-line px-5 py-3 bg-bg-soft/50">
          <div className="flex items-center gap-3">
            ...
            <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-ink-hush">veloseller / dashboard</span>
          </div>
          <span className="font-mono text-[10px] text-ink-hush">обновлено · 2 мин назад</span>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-px bg-line">
          <div className="lg:col-span-8 bg-paper p-5">
            ...
            <div className="mt-5 rounded-xl border border-line p-4 bg-paper">
          ...
          <div className="lg:col-span-4 bg-paper p-5">
            <div className="font-mono text-[10px] uppercase tracking-widest text-ink-hush mb-3">Сегменты SKU</div>
            <div className="space-y-2.5">
              {segments.map((s) => (
                <div key={s.name} className="flex items-center gap-3">
                  <span className="size-2 rounded-full" style={{ background: s.color }} />
                  <span className="font-mono text-xs text-ink-soft uppercase tracking-wider w-16">{s.name}</span>
                  <div className="flex-1 h-1.5 bg-line rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${s.value}%`, background: s.color }} />
                  </div>
                  <span className="font-mono text-xs text-ink-muted tabular w-7 text-right">{s.value}</span>
                </div>
              ))}
            </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={[
                    ...
                    <Bar dataKey="v" radius={[3,3,0,0]}>
                      {[0,1,2,3,4].map((i) => (
                        <Cell key={i} fill={i < 2 ? "#84cc16" : i < 4 ? "#0ea5e9" : "#a5ada3"} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
          ...
          <div className="lg:col-span-12 bg-paper p-5">
            ...
            <div className="divide-y divide-line border border-line rounded-xl overflow-hidden">
              {skuRows.map((r) => (
                <div key={r.sku} className="grid grid-cols-12 items-center gap-3 px-4 py-2.5 hover:bg-bg-soft transition">
                  <span className="col-span-3 font-mono text-[11px] text-ink-hush">{r.sku}</span>
                  <span className="col-span-5 text-[13px] text-ink-soft">{r.name}</span>
                  <span className="col-span-2 font-mono text-[13px] text-lime-deep tabular font-semibold">{r.tv.toFixed(2)}</span>
                  <span className="col-span-1 font-mono text-[11px] text-ink-hush tabular text-right">{r.cov}д</span>
  ...
  return (
    <div className="bg-paper p-4">
      ...
      <div className="mt-1.5 font-display tabular" style={{ color, fontSize: "1.7rem", lineHeight: 1, letterSpacing: "-0.03em" }}>
        {value}
        {suffix && <span className="text-base text-ink-hush ml-1">{suffix}</span>}
      </div>
    </div>
  );
}
...
function Pill({ children, tone }: { children: React.ReactNode; tone: "warn" | "bad" }) {
  ...
  return (
    <span className={`inline-block font-mono text-[9.5px] uppercase tracking-widest px-1.5 py-0.5 rounded border font-semibold ${cls}`}>
      {children}
    </span>
  );
}
