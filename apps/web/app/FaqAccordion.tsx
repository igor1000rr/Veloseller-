...
const items = [
  { q: "Сколько по времени занимает подключение?", a: "5-10 минут. Для Google Sheet вставить ссылку. Для Ozon/WB выдать read-only API ключ в личном кабинете маркетплейса." },
  { q: "Через сколько будет видна польза?",        a: "Первые TVelo расчёты и health-score через 30 минут. Для точных прогнозов нужно 7 дней истории — так алгоритм выявляет паттерны продаж." },
  { q: "Что если у меня несколько магазинов на разных маркетплейсах?", a: "Добавьте каждый как отдельный источник. Growth поддерживает до 3 магазинов, Pro — безлимит." },
  ...
  { q: "Чем TVelo лучше обычной velocity из Excel?", a: "Обычный подход sales/days — неправда, если товар не был в стоке часть периода. TVelo = sales / in_stock_days. Разница легко доходит до 50%." },
  { q: "Есть ли триал? Каждая ли тарифная опция бесплатна?", a: "Да — 30 дней любого плана. Карта не требуется. Если не подошло — просто перестаёшь пользоваться." },
];
