  ...
  return (
    <div className="space-y-6 md:space-y-10">
      <header>
        ...
        <h1 className="mt-2 font-display text-2xl sm:text-3xl md:text-4xl tracking-tight font-medium">Обзор платформы</h1>
      ...
      <section>
        ...
        <div className="rounded-2xl border border-line bg-paper p-4 sm:p-5 md:p-6">
                ...
                <div key={i}>
                  <div className="flex items-baseline justify-between gap-3 mb-1">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <span className="font-mono text-[10px] text-ink-hush tabular shrink-0">0{i + 1}</span>
                      <span className="text-xs sm:text-sm text-ink-soft truncate">{s.label}</span>
                    </div>
                    <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                      {dropoff !== null && dropoff > 0 && (
                        <span className="font-mono text-[10px] text-rose">-{dropoff.toFixed(0)}%</span>
                      )}
                      <span className="font-display text-base sm:text-lg text-ink tabular font-medium">{s.value}</span>
                    </div>
                  </div>
                ...
                <Link key={r.id} href={`/admin/sellers/${r.id}` as any} className="flex items-center justify-between px-3 sm:px-4 md:px-5 py-3 hover:bg-bg-soft transition gap-3">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="font-mono text-[10px] text-ink-hush truncate">{new Date(r.created_at).toLocaleString("ru-RU")}</div>
                    </div>
                  </div>
                  ...
                  <div key={c.id} className="px-3 sm:px-4 md:px-5 py-3 hover:bg-orange/[0.06] transition">
                    <div className="flex items-start justify-between gap-3 flex-wrap sm:flex-nowrap">
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 text-xs text-orange break-words" title={c.last_error}>{c.last_error}</div>
                      </div>
  ...
  return (
    <div className="bg-paper border border-line rounded-2xl p-4 sm:p-5">
      <div className="flex items-center justify-between mb-3 gap-2">
        <h3 className="font-mono text-[10px] uppercase tracking-[0.18em] text-ink-hush truncate">{title}</h3>
        {badge && <span className="font-mono text-[10px] text-lime-deep font-semibold shrink-0">{badge}</span>}
      </div>
      {children}
    </div>
  );
