...
class Settings(BaseSettings):
    ...
    # Вес «обрамлённого» пропуска (есть реальные снапшоты ДО и ПОСЛЕ дыры) в missing-штрафе.
    # Суммарный расход за провал известен из снапшотов вокруг — теряется лишь посуточная
    # разбивка (платформенный даунтайм/миграция), поэтому штрафуем вполовину. Хвостовой
    # провал (после последнего реального дня — «не знаем, что сейчас») остаётся полным (1.0).
    bracketed_gap_weight: float = 0.5
