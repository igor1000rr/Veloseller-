...
from app.engine.store import (
    SkuHealthInput, SkuValue, concentration_50, demand_weight, warehouse_health_score,
)
...
def aggregate_store_metrics(skus: list[SkuMetricRow]) -> StoreMetricsAggregate:
    ...
    dem_items = [SkuValue(s.product_id, demand_weight(s.adjusted_velocity, s.median_30d_velocity, s.price)) for s in skus]
    dem_conc = concentration_50(dem_items) or None
    total_value = sum(s.stock_quantity * s.price for s in skus)
    frozen_value = sum(s.stock_quantity * s.price for s in skus if s.coverage_days is not None and s.coverage_days > 180)
    total_lost = sum(lost_revenue_fn(s.adjusted_velocity, s.stockout_days, s.price) for s in skus)
    health_inputs = [
        SkuHealthInput(
            product_id=s.product_id,
            health_score=int(s.sku_health_score) if s.sku_health_score is not None else 50,
            stock_quantity=s.stock_quantity, price=s.price,
    ...
    return StoreMetricsAggregate(
        total_sku_count=total, oos_sku_count=oos, low_stock_sku_count=low_stock,
        dead_inventory_sku_count=dead, inventory_concentration_50=inv_conc,
        demand_concentration_50=dem_conc, total_inventory_value=round(total_value, 2),
        store_frozen_inventory_value=round(frozen_value, 2), lost_revenue=round(total_lost, 2),
