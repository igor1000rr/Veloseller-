...
Файл: `/opt/veloseller-worker/.env` (или Coolify env, в зависимости от деплоя).

```bash
# AI парсинг прайса (обязательно для /radar/extract-brands)
# Приоритет: DEEPSEEK_API_KEY → OPENROUTER_API_KEY (fallback)
DEEPSEEK_API_KEY=sk-...
# Опционально: DEEPSEEK_MODEL=deepseek-chat (default, V3 chat)
#              DEEPSEEK_MODEL=deepseek-reasoner (R1, дороже но точнее)
...
### Замечание про DeepSeek (актуально на 29.05.2026)

OpenRouter перестал принимать платежи из РФ. DeepSeek работает напрямую
(api.deepseek.com), не блокируется в РФ. Цены DeepSeek V3 в ~3-4 раза
ниже чем у Claude Haiku — анализ прайса 200 строк обходится ~$0.01.

Оплата DeepSeek: международная карта (любая страна кроме РФ) или
криптовалюта через сторонний обменник. Регистрация — platform.deepseek.com.
...
1. **Игорь регистрируется на platform.deepseek.com** → пополняет баланс ($5 хватит на месяцы) → кладёт `DEEPSEEK_API_KEY` в worker env
2. **Игорь регистрируется на xmlriver.com** → депозит 1000₽ → кладёт `XMLRIVER_USER` + `XMLRIVER_KEY` в worker env
3. **Игорь создаёт OAuth-приложение на oauth.yandex.ru** → пишет в Yandex Direct support с ClientID → получает `YANDEX_WORDSTAT_OAUTH_TOKEN`
