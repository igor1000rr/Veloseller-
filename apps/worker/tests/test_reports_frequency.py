...
Обновлено 01.06.2026: используем critical_stock и weekly_report — активные
kinds. low_stock теперь deprecated и фильтруется до dow/frequency-проверки.
"""
...
class TestFrequencyWeekly:
    ...
    def test_weekly_runs_every_matching_day(self, monkeypatch):
        """Пн 11 мая (второй пн) — weekly всё равно срабатывает."""
        today = datetime(2026, 5, 11, 9, 0, tzinfo=timezone.utc)
        assert today.isoweekday() == 1
        assert today.day > 7

        subs = [{
            "seller_id": "weekly-s", "kind": "critical_stock", "channel": "email",
            "enabled": True,
            "params": {"day_of_week": 1, "coverage_days_threshold": 3},
            "frequency": "weekly",
        }]
    ...
    def test_frequency_none_defaults_to_weekly(self, monkeypatch):
        """Старые подписки без frequency — трактуются как weekly."""
        today = datetime(2026, 5, 11, 9, 0, tzinfo=timezone.utc)
        subs = [{
            "seller_id": "legacy-s", "kind": "critical_stock", "channel": "email",
            "enabled": True,
            "params": {"day_of_week": 1, "coverage_days_threshold": 3},
